...
create_dummy(){
    ...
    if [ -z "$ARABELO_ACCESS" ] || [ -z "$ASTEIN_ACCESS" ] || [ -z "$ANSHOVAH_ACCESS" ] || [ -z "$FDAESTR_ACCESS" ] || [ -z "$RPHUYAL_ACCESS" ]; then
        echo -e "${RED}Failed to create dummy users${RESET}"
        exit 1
    fi
    # Check if all IDs are there
    if [ -z "$ARABELO_ID" ] || [ -z "$ASTEIN_ID" ] || [ -z "$ANSHOVAH_ID" ] || [ -z "$FDAESTR_ID" ] || [ -z "$RPHUYAL_ID" ]; then
        echo -e "${RED}Failed to create dummy users${RESET}"
        exit 1
    fi
}
...
download_tests(){
    # DOWNLOAD
    echo "Downloading ${CSV_FILE} from google sheets..."
    curl -s -L -o ${CSV_FILE} 'https://docs.google.com/spreadsheets/d/11uhG7xBOxUAyQIAAKTghT1Bh5gUYxqQ2fPpRNr6fjds/export?exportFormat=csv'
    ...
    echo "Found ${TOTAL_TESTS} tests in ${CSV_FILE}"
...
expand_vars(){
    ...
    source dummy.env
    variables=("arabelo" "astein" "anshovah" "fdaestr" "rphuyal")
...
run_tests(){
    ...
    while IFS=',' read -r should_work expected short_description user method endpoint args; do
        ...
        if [[ "$user" == "NONE" ]]; then
            # Curl with no token
            cmd=(curl -s -k -o response.json -w "%{http_code}" -X $method "$BASE_URL$endpoint" \
        ...
        status=$(jq -r '.status // empty' response.json 2>/dev/null) || status=""
