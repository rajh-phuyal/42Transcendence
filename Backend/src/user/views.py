...
class RelationshipView(BaseAuthenticatedView):
    ...
    def post(self, request):
        ...
        if action == 'send':
            ...
            return error_response(self.return_unvalid_action_msg(request, allowed_actions), status_code=405)
    ...
    def validate_data(self, request, allowed_actions):
        ...
        if action not in allowed_actions:
            raise ValidationException(self.return_unvalid_action_msg(request, allowed_actions), status_code=405)
        if not target_id:
...
class ListFriendsView(BaseAuthenticatedView):
    ...
    def get(self, request, id):
        ...
        if is_blocking(target_user, user):
            return error_response(_("You are blocked by this user"), status_code=403)
